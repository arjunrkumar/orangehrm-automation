...
Run npx playwright test --project=chrome Running 4 tests using 2 workers FF·T 1) [chrome] › tests/login.spec.js:11:1 › logs in successfully with valid credentials ───────────── Error: expect(locator).toHaveText(expected) failed Locator: locator('h6.oxd-topbar-header-breadcrumb-module')
